// app/manage/(auth)/login/page.tsx
"use client";
...
function ManageLoginPageContent() {
  ...
  const [error, setError] = useState<string | null>(
    urlError === "OAuthNotAllowed"
      ...
      : urlError === "Unauthorized"
        ? "Your account does not have management access."
        : urlError
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Check role from session before redirecting
    const session = await getSession();
    const role = session?.user?.role;

    if (!role || !MANAGEMENT_ROLES.includes(role)) {
      setError("Your account does not have management access. Please use the student login.");
      setLoading(false);
      return;
    }
